...
async def on_message(ctx):
    ...
    if ctx.attachments:
        ...
        discord.Embed.set_author(name="name", icon_url=bot.user.avatar_url)
        imageurl=os.getenv('IMAGE_ON_DISCORD')
        webhook = DiscordWebhook(url=os.getenv('GUILDED_WEBHOOK'), content='**Discord | ' + displayname + '** ' + ctx.content + " " + " ".join(attachment_urls), avatar_url=imageurl)
    response = webhook.execute()
    print('"' + displayname + '" sent a message!')
